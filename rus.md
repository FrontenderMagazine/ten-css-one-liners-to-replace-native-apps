...
с таким же представлением, используя веб-стандарты. Если веб-стандарты не могут
 решить эту задачу, разработчикам ничего не остаётся, как использовать
...
Это уже достаточно могущественная строчка кода, однако мы можем сделать её ещё лучше;
...
демонстрации эффекта, представленного на скриншотах выше. К тому же она
 работает в современных браузерах (чего нельзя пока сказать про следующие
примеры).
...
Код выше смещает строку с указанием автора, «By Collette Jackson», на одну
 ...
 должна располагаться в первой колонке, благодаря коду она окажется во второй (как в вырезке из газеты). Чтобы это
...
мешает.
Точно так же, я думаю, что реклама на устройствах с пролистываемым представлением будет менее назойливой, чем на устройствах с прокруткой.

## Смещённые цитаты

Последний пример контента, который может быть смещён — это цитаты.
Цитата — это выдержка из статьи, набранная большим шрифтом и расположенная в каком-то определённом месте страницы. В этом примере цитата находится посередине второй колонки:
...
отступив на сколько строк. Это будет полезно и для сгибаемых
экранов будущего.
